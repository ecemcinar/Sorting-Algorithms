...
public class HeapSort {
	...
	private void maxHeapify(int[] arrayToSort,int n,int i) { //          8
		...
		// ornegin ilk cagirmada max=i=0(array[0]=8) iken, left_child=1 (arr[1]=10] ve right_child=2 (arr[2]=4) olucaktir.
		int max = i; // root						
		int left = i*2+1; // left child index
		int right = i*2 +2; // right child index
		
		// left child > its parent
		...
		// ornegin i=3 oldugunda yani array[i]=5'e geldinide, 5 leaf ve childi exist degil. fakat en basta tanimlama yaparken left=11 seklinde tanimlamistik. iste bu left<n kontrolu bunun icindir.
			max = left; // left child parentinden buyuk ise, max'e left childin indeksi atanir
		}
		if(right<n && arrayToSort[right]> arrayToSort[max]) { // right child parentindan buyukse
			max= right; // left child durumunda oldugu gibi.
		}
